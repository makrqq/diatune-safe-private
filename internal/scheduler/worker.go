...
func New(settings config.Settings, svc *service.Service) *Worker {
	...
	if strings.TrimSpace(settings.Timezone) != "" {
		...
		if err != nil {
			log.Printf("некорректный TIMEZONE=%s, используем UTC", settings.Timezone)
		} else {
			location = loc
		}
	}
	...
	if w.settings.AutoAnalysisEnabled && len(analysisIDs) > 0 {
		...
		for _, patientID := range analysisIDs {
			...
			_, err := w.cron.AddFunc(spec, func() {
				...
				if err != nil {
					log.Printf("плановый анализ завершился с ошибкой patient_id=%s err=%v", pid, err)
				}
			})
			if err != nil {
				return err
			}
			log.Printf("запланирован анализ patient_id=%s spec=%s", pid, spec)
		}
	}
...
func (w *Worker) configureDailyRecommendations(defaultPatientIDs []string) error {
	...
	if strings.TrimSpace(w.settings.TelegramBotToken) == "" {
		log.Printf("ежедневные рекомендации включены, но TELEGRAM_BOT_TOKEN пустой; пропуск")
		return nil
	}
	if len(w.settings.TelegramAllowedUserIDs) == 0 {
		log.Printf("ежедневные рекомендации включены, но TELEGRAM_ALLOWED_USER_IDS пустой; пропуск")
		return nil
	}
	...
	if len(patientIDs) == 0 {
		log.Printf("ежедневные рекомендации включены, но не указаны patient_id; пропуск")
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("ошибка инициализации Telegram-бота: %w", err)
	}
	...
	_, err = w.cron.AddFunc(spec, func() {
		for _, patientID := range patientIDs {
			...
			if err != nil {
				log.Printf("ошибка ежедневной рекомендации patient_id=%s err=%v", patientID, err)
				continue
			}
	...
	log.Printf("запланированы ежедневные рекомендации spec=%s time=%s patients=%d users=%d",
...
func (w *Worker) configureWeeklyStats(defaultPatientIDs []string) error {
	...
	if strings.TrimSpace(w.settings.TelegramBotToken) == "" {
		log.Printf("еженедельная статистика включена, но TELEGRAM_BOT_TOKEN пустой; пропуск")
		return nil
	}
	if len(w.settings.TelegramAllowedUserIDs) == 0 {
		log.Printf("еженедельная статистика включена, но TELEGRAM_ALLOWED_USER_IDS пустой; пропуск")
		return nil
	}
	...
	if len(patientIDs) == 0 {
		log.Printf("еженедельная статистика включена, но не указаны patient_id; пропуск")
		return nil
	}
	...
	_, err = w.cron.AddFunc(spec, func() {
		for _, patientID := range patientIDs {
			...
			if err != nil {
				log.Printf("ошибка еженедельной статистики patient_id=%s err=%v", patientID, err)
				continue
			}
	...
	log.Printf("запланирована еженедельная статистика spec=%s day=%s time=%s patients=%d users=%d",
	...
	for _, userID := range w.settings.TelegramAllowedUserIDs {
		for _, chunk := range chunks {
			...
			if _, err := bot.Send(msg); err != nil {
				log.Printf("не удалось отправить сообщение user_id=%d err=%v", userID, err)
				break
			}
		}
	}
...
func parseDailyTime(raw string) (int, int) {
	...
	if err != nil {
		log.Printf("некорректный DAILY_RECOMMENDATION_TIME=%s, используем 22:00", raw)
		return 22, 0
	}
...
func parseWeekday(raw string) int {
	...
	case "sat", "saturday", "сб":
		return 6
	default:
		log.Printf("некорректный WEEKLY_STATS_DAY=%s, используем mon", raw)
		return 1
	}
}
