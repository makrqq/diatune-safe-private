...
func (r *Runner) handleCommand(ctx context.Context, bot *tgbotapi.BotAPI, chatID int64, userID int64, cmd string, args []string) {
	...
	case "/analyze":
		...
		r.send(bot, chatID, formatReport(report))
	case "/latest":
		patientID := fmt.Sprintf("tg-%d", userID)
		if len(args) > 0 {
			patientID = args[0]
	...
	case "/pending":
		...
		for _, rec := range recs {
			lines = append(lines, formatRecommendation(rec))
		}
...
func formatReport(report domain.AnalysisReport) string {
	lines := []string{
		fmt.Sprintf("Report #%v [%s]", derefInt64(report.RunID), report.PatientID),
		fmt.Sprintf("Generated: %s", report.GeneratedAt.Local().Format("2006-01-02 15:04")),
		fmt.Sprintf("Period: %s..%s", report.PeriodStart.Format("2006-01-02"), report.PeriodEnd.Format("2006-01-02")),
		fmt.Sprintf("Global hypos: %d", report.GlobalHypoEvents),
	}
...
func formatRecommendation(rec domain.Recommendation) string {
	status := "OPEN"
	if rec.Blocked {
		status = "BLOCKED"
	}
	...
	line := fmt.Sprintf("#%s %s %s %s: %.2f -> %.2f (%s%.1f%%, conf=%.2f)",
		id, status, rec.BlockName, strings.ToUpper(string(rec.Parameter)), rec.CurrentValue, rec.ProposedValue,
		sign, rec.PercentChange, rec.Confidence)
